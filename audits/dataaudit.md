...
description: >
  ...
  fix plan, fix plan, Builder handoff, and DB backup safety gate (READ-ONLY auditor — never writes. Destructive apply is a Builder job). Outputs audits/.dataaudit/verdict.json consumed by
  /apiaudit for schema-to-contract validation. Score /320. Preamble v1.0 compliant.
  Audit → Plan → Builder handoff (READ-ONLY).
