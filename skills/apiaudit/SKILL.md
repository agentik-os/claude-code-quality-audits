...
## Recipe
1. Confirm tenant and READ-ONLY (refuse product edits unless `apiaudit` allows `--fix` and the user spelled it).
2. `apiaudit` special cases: `retentionaudit` never edits; `agentaudit` refuses `--fix`; `secaudit`/`agentaudit` never emit exploit PoCs.
3. Read and follow the forensic protocol in `audits/apiaudit.md` (same repo as this skill).
4. Label each finding `tool-backed` | `llm-judgment` | `inventory-only`.
5. Emit the 8-file output contract under `audits/.apiaudit/`.
