...
## When to use
Orphans, migrations, referential integrity. AGK Audit never writes. Destructive apply is a Builder job.
...
2. Obey the **AGK-AUDIT-OVERRIDE-V2** block in the first 100 lines of `audits/dataaudit.md` before the rest of that file.
...
5. Emit the 8-file output contract under `audits/.dataaudit/` with `mode=readonly`.
